...
from .raster import get_array_from_tiff
...
def split_geom(geometry, tile_size, tile_overlay, resolution=None,
    ...
    if use_projection_units is False:
        ...
        tmp_overlay = tile_overlay*resolution[0]
    else:
    ...
    x_extent = xmax - xmin
    y_extent = ymax - ymin
    x_steps = np.ceil(x_extent/(tmp_tile_size[1]-tmp_overlay))
    y_steps = np.ceil(y_extent/(tmp_tile_size[0]-tmp_overlay))
    x_mins = np.arange(xmin, xmin + (tmp_tile_size[1]-tmp_overlay)*x_steps,
                       tmp_tile_size[1] - tmp_overlay)
    y_mins = np.arange(ymin, ymin + (tmp_tile_size[0]-tmp_overlay)*y_steps,
                       tmp_tile_size[0] - tmp_overlay)
    tile_bounds = [
        (i, j, i+tmp_tile_size[1], j+tmp_tile_size[0])
        for i in x_mins for j in y_mins]
    print("count of tile bounds", len(tile_bounds))
    return tile_bounds
...
def flatten_file(file):
    f = get_array_from_tiff(file)
    b1 = f[0].flatten()
    b2 = f[1].flatten()
    b3 = f[2].flatten()
    b4 = f[3].flatten()
    res = np.stack((b1,b2,b3,b4), axis=-1)
    return res 